...
class Admin_model extends CI_Model
{
    ...
    function getHealthHistory($patientid)
    {
        $this->db->select('*');
        $this->db->from('contact_info');
        $this->db->where('contact_info.patient_id', $patientid); // Specify the table alias for clarity
        $this->db->join('healthinfo', 'healthinfo.patient_id = contact_info.patient_id');
        $this->db->join('familyinfo', 'familyinfo.patient_id = contact_info.patient_id');
        $this->db->join('additionalinfo', 'additionalinfo.patient_id = contact_info.patient_id');

        $query = $this->db->get();

        if ($query->num_rows() > 0) {
            return $query->row(); // Assuming you want a single row with data from all tables
        } else {
            log_message('error', 'No health history data found for patient ' . $patientid);
    ...
    public function get_contact_info($data)
    {
        ...
        $query = $this->db->get('contact_info');
        return $query->result(); // Return results as objects
    }
    public function get_health_info($data)
    {
        $patient_id = $data['patient_id'];
        $this->db->where('patient_id', $patient_id);
        $query = $this->db->get('healthinfo');
        return $query->result(); // Return results as objects
    }
    public function get_family_info($data)
    {
        $patient_id = $data['patient_id'];
        $this->db->where('patient_id', $patient_id);
        $query = $this->db->get('familyinfo');
        return $query->result();
    }
    ...
    public function get_additional_info($data)
    {
        ...
        $this->db->where('patient_id', $patient_id);
        // $query = $this->db->get('contact_info');
        $query = $this->db->get('additionalinfo');
        return $query->result(); // Return results as objects
    }
    ...
    function puthealthinfo($data)
    {
        ...
        if ($query->num_rows() > 0) {
            // Update the existing row
            $this->db->where('patient_id', $patient_id);
            $this->db->update('healthinfo', $data);
        } else {
            // Insert a new row
            $this->db->insert('healthinfo', $data);
        }
    }
    function putfamilyinfo($data)
    {
        $patient_id = $data['patient_id'];
        // Check if a row with the same patient_id exists
        $this->db->where('patient_id', $patient_id);
        ...
        if ($query->num_rows() > 0) {
            // Update the existing row
            $this->db->where('patient_id', $patient_id);
            $this->db->update('familyinfo', $data);
        } else {
            // Insert a new row
            $this->db->insert('familyinfo', $data);
        }
    }
    function putadditionalinfo($data)
    {
        $patient_id = $data['patient_id'];
