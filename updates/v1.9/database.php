...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE `settings` ADD `profile_verification` VARCHAR(11) NULL DEFAULT '0' AFTER `expire_reminder`, ADD `verification_items` MEDIUMTEXT NULL AFTER `profile_verification`;");

        mysqli_query($connection, "ALTER TABLE `users` ADD `is_verified` INT NULL DEFAULT '0' AFTER `signature`;");


        // import database table
        $query = '';
          $sqlScript = file('sql/verification_files.sql');
        ...
        ('admin', 'Doctors Verification', 'doctors-verification', 'Doctors Verification'),
        ('admin', 'Enable Verification', 'enable-verification', 'Enable Verification'),
        ('admin', 'Enable to force all doctors to verify their profile to submit the required docments by admin', 'enable-verification-details', 'Enable to force all doctors to verify their profile to submit the required docments by admin'),
        ('admin', 'In this section you can add required doctors certificate/document name and files upload option will be shown on doctors pane', 'enable-verification-suggestion', 'In this section you can add required doctors certificate/document name and files upload option will be shown on doctors panel to verify their account via admin'),
        ('admin', 'Account Verification', 'account-verification', 'Account Verification'),
        ('admin', 'Documents submitted successfully', 'documents-submitted-successfully', 'Documents submitted successfully'),
        ('admin', 'Your files are in under review, Once verify is done you will know about your status', 'verify-under-process-short', 'Your files are in under review, Once verify is done you will know about your status'),
        ('admin', 'Your account is now verified', 'your-account-is-now-verified', 'Your account is now verified'),
        ('admin', 'Verify Profile', 'verify-profile', 'Verify Profile'),
        ('admin', 'Verify Documents', 'verify-documents', 'Verify Documents'),
        ('admin', 'Verify Your Account', 'verify-your-account', 'Verify Your Account'),
        ('admin', 'To verify your account you need to submit some required files, Please click below link to go the verification page.', 'verify-account-hints', 'To verify your account you need to submit some required files, Please click below link to go the verification page.'),
        ('admin', 'Account Verified', 'account-verified', 'Account Verified');");
      ...
      $redir = str_replace('updates/v1.9/', '', $redir);
